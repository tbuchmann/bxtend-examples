	...
	override void sourceToTarget() {
		sourceModel.allContents.filter(typeof(Dependency))
			.forEach[ d |
				...
				// Name encodes the predecessor→successor relationship; duration stores the lag/lead offset
		...
		targetModel.allContents.filter(typeof(Activity)).filter(a | a.name.contains("->"))
			.forEach[ a |
				...
				if (cpmSourceActivity.sourceEvent == cpmSourceEvent && cpmTargetActivity.sourceEvent == cpmTargetEvent) 
					sourceDependency.dependencyType = DependencyType.START_START
				else if (cpmSourceActivity.sourceEvent == cpmSourceEvent && cpmTargetActivity.targetEvent == cpmTargetEvent)
					sourceDependency.dependencyType = DependencyType.START_END
				else if (cpmSourceActivity.targetEvent == cpmSourceEvent && cpmTargetActivity.sourceEvent == cpmTargetEvent)
					sourceDependency.dependencyType = DependencyType.END_START
				else
					sourceDependency.dependencyType = DependencyType.END_END
				sourceDependency.predecessor = ganttSourceActivity as gantt.Activity
				sourceDependency.successor = ganttTargetActivity as gantt.Activity
				// Offset is stored in the CPM activity's duration field
				sourceDependency.offset = a.duration
				sourceDependency.diagram = a.network.corrModelElem.sourceElement as GanttDiagram
			]
