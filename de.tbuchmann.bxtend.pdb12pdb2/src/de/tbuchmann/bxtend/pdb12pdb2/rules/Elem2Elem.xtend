...
abstract class Elem2Elem {
	 ...
	 */
	protected static Map<Corr, String> corrToBirthday = newHashMap
	protected static Map<Corr, String> corrToPlaceOfBirth = newHashMap
	protected static Map<Corr, String> corrToId = newHashMap
	...
	new(Resource src, Resource trgt, Resource corr) {
		...
		corrModel = corr
		ruleID = "base"
		// Rebuild the in-memory index from persisted correspondences so that
		// incremental runs can locate existing Corr entries in O(1).
	...
	def getOrCreateCorrModelElement(EObject obj, String description) {
		...
		if (corr === null) {
			corr = corrFactory.createBasicElem => [
				if (obj.eClass.EPackage instanceof Pdb1Package)
					sourceElement = obj
				if (obj.eClass.EPackage instanceof Pdb2Package)
					targetElement = obj
				desc = description
			]
