...
public abstract class Elem2Elem {
   ...
   */
  protected final SetsFactory sourceFactory = SetsFactory.eINSTANCE;

  /**
   * Factory for creating new target-side (OSets) model elements.
   */
  protected final OsetsFactory targetFactory = OsetsFactory.eINSTANCE;

  /**
   * Factory for creating new correspondence model elements ({@link Corr}, {@link Transformation}).
   */
  protected final Set2osetFactory corrFactory = Set2osetFactory.eINSTANCE;

  /**
   * EMF package descriptor for the source metamodel; used to resolve {@link EClass} literals.
   */
  protected final SetsPackage sourcePackage = SetsPackage.eINSTANCE;

  /**
   * EMF package descriptor for the target metamodel; used to resolve {@link EClass} literals.
   */
  protected final OsetsPackage targetPackage = OsetsPackage.eINSTANCE;
  ...
  public Corr getOrCreateCorrModelElement(final EObject obj, final String description) {
    ...
    boolean _equals = Objects.equals(corr, null);
    if (_equals) {
      ...
      final Procedure1<BasicElem> _function = (BasicElem it) -> {
        ...
        if ((_ePackage instanceof SetsPackage)) {
          it.setSourceElement(obj);
        }
        EPackage _ePackage_1 = obj.eClass().getEPackage();
        if ((_ePackage_1 instanceof OsetsPackage)) {
